...
function CustomModal({ onSet, onClose }) {
  ...
  const clampHrs = (v) => Math.max(0, Math.min(5, v));
  const clampMins = (v) => Math.max(0, Math.min(59, v));
  ...
  const handleSet = () => { if (canSet) onSet(totalMins); };

  const spinUp = (field) => {
    if (field === "h") setHrs((v) => clampHrs(v + 1));
    else setMins((v) => (v >= 59 ? 0 : v + 1));
  };
  const spinDown = (field) => {
    if (field === "h") setHrs((v) => clampHrs(v - 1));
    else setMins((v) => (v <= 0 ? 59 : v - 1));
  };

  const handleHrsInput = (e) => {
    const v = parseInt(e.target.value, 10);
    setHrs(isNaN(v) ? 0 : clampHrs(v));
  };
  const handleMinsInput = (e) => {
    const v = parseInt(e.target.value, 10);
    setMins(isNaN(v) ? 0 : clampMins(v));
  };
      ...
      <div className="cm-panel">
        ...
        <div className="cm-pickers">
          <div className="cm-field">
            <button className="cm-arrow" onClick={() => spinUp("h")} tabIndex={-1}>
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
                <path d="M1 7L6 2L11 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </button>
            <input
              className="cm-digit-input"
              type="number"
              min={0} max={5}
              value={hrs}
              onChange={handleHrsInput}
            />
            <button className="cm-arrow" onClick={() => spinDown("h")} tabIndex={-1}>
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
          ...
          <div className="cm-field">
            <button className="cm-arrow" onClick={() => spinUp("m")} tabIndex={-1}>
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
                <path d="M1 7L6 2L11 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </button>
            ...
            />
            <button className="cm-arrow" onClick={() => spinDown("m")} tabIndex={-1}>
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
                <path d="M1 1L6 6L11 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </button>
        ...
        <div className="cm-actions">
          ...
          <button className={`cm-confirm ${canSet ? "ready" : ""}`} onClick={handleSet} disabled={!canSet}>
            Set timer
          </button>
        </div>
      </div>
...
export default function StudyFocus() {
  ...
  const handleCustomSet = (minutes) => { setPreset(minutes); setShowCustom(false); };
  const handleStartPause = () => { if (isDone) return; setIsRunning((r) => !r); };
  const handleReset = () => { setIsRunning(false); setIsDone(false); setRemaining(totalSeconds); };
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cm-panel {
          background: #0e0d0c; border: 0.5px solid #2a2825;
          padding: 3rem 3.5rem 2.5rem; min-width: 340px;
          animation: cm-slide-up 0.22s cubic-bezier(0.16,1,0.3,1);
          display: flex; flex-direction: column; align-items: center;
        }
        @keyframes cm-slide-up { from{opacity:0;transform:translateY(18px)} to{opacity:1;transform:translateY(0)} }
        .cm-eyebrow {
          font-family: 'DM Sans', sans-serif; font-size: 10px; font-weight: 500;
          letter-spacing: 0.32em; text-transform: uppercase; color: #3a3733; margin-bottom: 2.5rem;
        }
        ...
        .cm-arrow {
          ...
          color: #2a2825; padding: 6px 10px; transition: color 0.15s;
          display: flex; align-items: center; justify-content: center;
        }
        ...
        .cm-digit-input {
          ...
          outline: none; text-align: center; width: 110px; padding: 0 0 6px;
          -moz-appearance: textfield; transition: border-color 0.2s;
        }
        .cm-digit-input::-webkit-inner-spin-button,
        .cm-digit-input::-webkit-outer-spin-button { -webkit-appearance: none; }
        .cm-digit-input:focus { border-bottom-color: #c8533a; }
        .cm-unit {
          font-family: 'DM Sans', sans-serif; font-size: 10px; font-weight: 400;
          letter-spacing: 0.22em; text-transform: uppercase; color: #2a2825;
          position: absolute; bottom: 0; left: 50%; transform: translateX(-50%);
          white-space: nowrap;
        }
        .cm-colon {
          font-family: 'Barlow Condensed', sans-serif; font-weight: 800;
          font-size: 72px; color: #1e1c1a; line-height: 1;
          padding-bottom: 18px; margin: 0 4px;
        }
        .cm-summary {
          font-family: 'DM Sans', sans-serif; font-size: 11px; font-weight: 400;
          letter-spacing: 0.18em; color: #3a3733; text-align: center; min-height: 1.4em;
        }
        .cm-actions { display: flex; gap: 1.5rem; margin-top: 2.25rem; align-items: center; }
        .cm-cancel {
          font-family: 'DM Sans', sans-serif; font-size: 10px; font-weight: 400;
          letter-spacing: 0.22em; text-transform: uppercase;
          color: #3a3733; background: transparent; border: none; cursor: pointer;
        ...
        .cm-confirm {
          ...
          color: #3a3733; background: transparent;
          border: 0.5px solid #2a2825; padding: 12px 28px;
      ...
      <div className="sf-root">
        ...
        <div className="sf-inner">
          ...
          <div className="sf-controls">
            <button className="sf-btn-primary" onClick={handleStartPause} disabled={isDone}>
      ...
      {showCustom && <CustomModal onSet={handleCustomSet} onClose={() => setShowCustom(false)} />}
    </>
  );
}